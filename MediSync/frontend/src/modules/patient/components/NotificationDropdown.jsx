...
export default function NotificationDropdown({ notifications, onMarkRead, onMarkAllRead, onDismiss, isOpen, onClose }) {
  ...
  const getNotifColor = (type) => { 
    const c = { 
      appointment: dark ? "bg-blue-900 text-blue-300" : "bg-blue-100 text-blue-600", 
      patient: dark ? "bg-green-900 text-green-300" : "bg-green-100 text-green-600", 
      alert: dark ? "bg-red-900 text-red-300" : "bg-red-100 text-red-600", 
      system: dark ? "bg-gray-700 text-gray-300" : "bg-gray-100 text-gray-600", 
      security: dark ? "bg-purple-900 text-purple-300" : "bg-purple-100 text-purple-600" 
    }; 
    return c[type] || c.system; 
  };
      ...
      {isOpen && (
        <>
          ...
          <motion.div 
            ...
            className={`absolute right-0 top-full mt-2 w-96 max-w-[calc(100vw-2rem)] ${dark ? "bg-[#0a0c10] border-[#1e293b]" : "bg-white border-slate-100"} rounded-2xl shadow-2xl border z-50 overflow-hidden`}
          >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-teal-50/70 text-[11px] font-medium">
                      {unreadCount > 0 ? `${unreadCount} unread notifications` : "Everything is up to date!"}
                    </p>
                  </div>
                </div>
                {unreadCount > 0 && (
