    ...
    <AnimatePresence>
      {isOpen && (<>
        ...
        <motion.div initial={{ opacity: 0, y: -10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -10, scale: 0.95 }} transition={{ duration: 0.15 }} className={`absolute right-0 top-full mt-2 w-72 ${dark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-100"} rounded-2xl shadow-2xl border z-50 overflow-hidden`}>
          <div className="p-5 bg-gradient-to-r from-[#2da0a8] to-[#258a91]"><div className="flex items-center gap-3"><div className="w-12 h-12 bg-white bg-opacity-20 rounded-full flex items-center justify-center text-white font-bold text-lg">{(apiService.getUserDisplayName() || 'A').charAt(0).toUpperCase()}</div><div><p className="text-white font-bold">{apiService.getUserDisplayName() || 'User'}</p><p className="text-blue-200 text-xs">{apiService.getUserRole()?.toUpperCase() || 'MEMBER'}</p></div></div></div>
