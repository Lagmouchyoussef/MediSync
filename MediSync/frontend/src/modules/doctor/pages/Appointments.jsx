...
import { useTheme, getPatientId } from "../components/DoctorShared";
import apiService from "../../../core/services/api";
import { Icon, Badge, Modal } from "../components/DoctorUI";
...
export default function Appointments({ activeTab: externalActiveTab, setActiveTab: setExternalActiveTab, invitations = [], setInvitations, patients = [] }) {
  ...
  const handleDeleteAppointment = async () => {
    ...
    try {
      await apiService.deleteAppointment(deleteConfirm.id);
      setInvitations(prev => prev.filter(inv => inv.id !== deleteConfirm.id));
      await apiService.createActivity("Appointment Deleted", `Appointment ID ${deleteConfirm.id} was permanently deleted.`, "danger");
        ...
        <div className="text-center py-4 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className={`text-sm font-medium ${textSecondary} px-4 leading-relaxed`}>
              Permanently delete this appointment invitation? This action cannot be undone.
            </p>
          </div>
